...
def run(config: BotConfig) -> None:
    ...
    time.sleep(0.3)
    emit_log("info", "Baixando manifestos de carga do dia…", eid)
    time.sleep(1.0)
    emit_log("info", "Organizando por navio e destino…", eid)
    time.sleep(0.8)
...
if __name__ == "__main__":
    ...
    config = BotConfig(bot_id=args.bot_id, name="Extração de Manifesto", openport_token=args.openport_token)
    run(config)
